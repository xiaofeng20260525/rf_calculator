...
>
> **v2.3 公式勘误版** — 修正了 v2.2 中的 7 项公式错误，详见文末 [变更记录](#变更记录-v23)。
...
- **当 RL > Z₀ 时（高阻匹配）：**
  ...
  - Bp = Q / RL（并联电纳）
  - 负载电抗 XL 被吸收到并联支路处理（Bp 需修正为 Bp' = Q/RL + BL，其中 BL = −XL/(RL²+XL²)）
...
**原理：** Cohn 模型，对称带状线。设接地板间距 b = 2h。

**计算公式：**

- **窄带近似（W/b < 0.35 即 W/h < 0.7）：**
  - Z₀ = 60 / √εr × ln(8h / πW)

- **宽带近似（W/b ≥ 0.35 即 W/h ≥ 0.7）：**
  - Z₀ = 94.15 / [√εr × (W/b + 0.441)] = 94.15 / [√εr × (W/(2h) + 0.441)]
...
- Z₀ = 30π / √εr_eff × K(k') / K(k)
  - 由上式，K(k')/K(k) ≈ ln[2(1 + √k')/(1 − √k')] / π
  - 代入得：Z₀ ≈ 30 / √εr_eff × ln[2(1 + √k')/(1 − √k')]
- εr_eff ≈ (εr + 1) / 2
...
- 带状线导体损耗：αc = 8.686 × Rs / (2 × Z₀ × Weff) × [1 + 2W/(πb)]（dB/m），b = 2h
...
- Hn_suppression = Hn 相对于基波的抑制量（dBc，取负值）
...
> 以下为 v2.2 → v2.3 的公式勘误修复，**请同步修正 Python 代码中的对应实现**。

| # | 位置 | 问题 | v2.2 (错误) | v2.3 (修正) |
|---|------|------|------------|------------|
| 1 | §1.1 L-Match 高阻 | Xs 符号与幅值 | `Xs = −(Q×Z₀+XL)` | `Xs = ±(Q×Z₀)`，XL 经 Bp 吸收 |
| 2 | §1.2 Single-Stub | tan(βd) 公式 | `(BL ± √[...])` | `(−BL ± √[...])`，且判别式修正为 `GL×(BL²+(1−GL)²)` |
| 3 | §3.1 微带线 | εr_eff 未区分 W/h | 仅给出 W/h≤1 的公式 | 新增 W/h>1 的简化公式（去掉 `0.04×(1−W/h)²` 项） |
| 4 | §3.2 带状线窄带 | ln 参数错误 | `ln(4h/πW)` | `ln(8h/πW)` |
| 5 | §3.2 带状线宽带 | W/h 应为 W/(2h) | `W/h + 0.441` | `W/(2h) + 0.441` |
| 6 | §4.1 IMD3 电平 | IIP3→OIP3 混淆 | `3×P_out − 2×IIP3` | `3×P_out − 2×OIP3` |
| 7 | §4.1 C/I | IIP3→OIP3 混淆 | `2×(IIP3 − P_out)` | `2×(OIP3 − P_out)` |
| 8 | §4.5 ACLR | IIP3→OIP3 混淆 | `2×(IIP3 − Pout) − ...` | `2×(OIP3 − Pout) − 1.2×PAPR` |

### 额外改进

- §3.3 CPW：补充了 `K(k')/K(k) ≈ ln[...]/π` 的推导，方便直接使用
- §3.6.1 带状线导体损耗：将 `[1+2W/(πh)]` 改为更标准的 `[1+2W/(πb)]`（b=2h）
- §4.4 TX 谐波：明确标注 `Hn_suppression` 取负值（dBc）
- 版本号升级为 v2.3
